...
import { rotateInboxApiKey } from "@/app/dashboard/_actions/inboxes";

export function PublishableKeyCard({
  inboxId,
  inboxName,
  audience,
  purpose,
  ...
  function rotate() {
    if (
      !confirm(
        `Rotate the publishable key for "${inboxName}"? Any embed using the old key will stop working.`,
      )
    )
      return;
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <div className="text-[12px] text-deep/60 mt-0.5">
            {purpose} · {audience} · safe to embed in browsers
          </div>
        </div>
